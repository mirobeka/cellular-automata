        ...
        if len(self) > max_len:
            for k in list(self.keys()):
                if self[k]['iteration'] < min_iter:
                    del self[
                        k]  # only deletes one item with k as key, should delete all?
...
class BestSolution(object):
    ...
    def update(self, arx, xarchive=None, arf=None, evals=None):
        ...
        if type(arx) == BestSolution:
            ...
            self.x_geno = xarchive[self.x][
                'geno'] if xarchive is not None else None
...
class BoundPenalty(object):
    ...
    def repair(self, x, bounds=None, copy=False, copy_always=False):
        ...
        if bounds not in (
        None, [None, None], (None, None)):  # solely for effiency
    ...
    def __call__(self, x, archive, gp):
        ...
        # CAVE: this does not work with already repaired values!!
            ...
            pen.append(
                sum(self.gamma * ((xinbounds - xpheno) / fac) ** 2) / len(xi))
    ...
    def update(self, function_values, es, bounds=None):
        ...
        if bounds is None:
            bounds = self.bounds
        if bounds is None or (bounds[0] is None and bounds[
            1] is None):  # no bounds ==> no penalty
            return self  # len(function_values) * [0.0]  # case without voilations

        N = es.N
        ### prepare
        # compute varis = sigma**2 * C_ii
        varis = es.sigma ** 2 * array(
            N * [es.C] if np.isscalar(es.C) else (  # scalar case
                                                    es.C if np.isscalar(es.C[
                                                        0]) else  # diagonal matrix case
                                                    [es.C[i][i] for i in xrange(
                                                        N)]))  # full matrix case

        # dmean = (es.mean - es.gp.into_bounds(es.mean)) / varis**0.5
        dmean = (es.mean - es.gp.geno(
            es.gp.into_bounds(es.gp.pheno(es.mean)))) / varis ** 0.5

        ### Store/update a history of delta fitness value
        fvals = sorted(function_values)
        l = 1 + len(fvals)
        val = fvals[3 * l // 4] - fvals[
            l // 4] # exact interquartile range apart interpolation
        val = val / np.mean(
            varis)  # new: val is normalized with sigma of the same iteration
        ...
        if len(self.hist) == 0:
            ...
            # initialize weights
        if (dmean.any() and (
            not self.weights_initialized or es.countiter == 2)):  # TODO
        ...
        if self.weights_initialized:
            ...
            if 1 < 3:  # this is better, around a factor of two  
                ...
                r = self.feasible_ratio(
                    solutions)  # has to be the averaged over N iterations
                self.gamma *= exp(np.max([N * [0], 0.3 - r], axis=0)) ** min(1,
                                                                             es.sp.mueff / 10 / N)
        es.more_to_write += list(
            self.gamma) if self.weights_initialized else N * [1.0]
        ### return penalty
...
class GenoPheno(object):
    ...
    def __init__(self, dim, scaling=None, typical_x=None, bounds=None,
        ...
        if tf is not None:
            ...
            # assert all(tf[0](tf[1](r)) - r > -1e-7)
            print(
                "WARNING in class GenoPheno: user defined transformations have not been tested thoroughly")
        else:
            self.tf_geno = None
            self.tf_pheno = None
        ...
        if fixed_values:
            if type(fixed_values) is not dict:
                raise _Error(
                    "fixed_values must be a dictionary {index:value,...}")
            if max(fixed_values.keys()) >= dim:
                raise _Error("max(fixed_values.keys()) = " + str(
                    max(fixed_values.keys())) +
                             " >= dim=N=" + str(
                    dim) + " is not a feasible index")
        ...
        if bounds:
            ...
            for i in (0, 1):
                if bounds[i] is not None:
                    bounds[i] = array(
                        dim * [bounds[i]] if np.isscalar(bounds[i]) else
                        [b for b in bounds[i]])
        ...
        if bounds[0] is not None:
            if len(bounds[0]) not in (1, len(y)):
                raise ValueError('len(bounds[0]) = ' + str(len(bounds[0])) +
                                 ' and len of initial solution (' + str(
                    len(y)) + ') disagree')
...
class CMAEvolutionStrategy(OOOptimizer):
    ...
    *pairwise selection* (where at most one of the two mirrors contribute to the 
    ...
    def stop(self, check=True):
        ...
        if (check and self.countiter > 0 and self.opts[
            'termination_callback'] and
                    self.opts['termination_callback'] != str(
                        self.opts['termination_callback'])):
            self.callbackstop = self.opts['termination_callback'](self)

        return self.stopdict(
            self if check else None)  # update the stopdict and return a Dict
    ...
    def __init__(self, x0, sigma0, inopts={}):
        ...
        del self.inputargs[
            'self'] # otherwise the instance self has a cyclic reference
        self.inopts = inopts
        opts = Options(
            inopts).complement()  # Options() == fmin([],[]) == defaultOptions()

        if opts['noise_handling'] and eval(opts['noise_handling']):
            raise ValueError(
                'noise_handling not available with class CMAEvolutionStrategy, use function fmin')
        if opts['restarts'] and eval(opts['restarts']):
            raise ValueError(
                'restarts not available with class CMAEvolutionStrategy, use function fmin')
        ...
        if self.mean.ndim == 2:
            print(
                'WARNING: input x0 should be a list or 1-D array, trying to flatten ' +
        ...
        self.sigma0 = sigma0
        if isinstance(sigma0,
                      str):  # TODO: no real need here (do rather in fmin)
            self.sigma0 = eval(
                sigma0)  # like '1./N' or 'np.random.rand(1)[0]+1e-2'
        if np.size(self.sigma0) != 1 or np.shape(self.sigma0):
            raise _Error(
                'input argument sigma0 must be (or evaluate to) a scalar')
        ...
        self.countiter = 0
        self.countevals = max((0, opts['verb_append'])) if type(
            opts['verb_append']) is not bool else 0
        self.ps = np.zeros(N)
        self.pc = np.zeros(N)
        ...
        if np.all(self.opts['CMA_teststds']):  # also 0 would not make sense
            ...
            if np.size(stds) != N:
                raise _Error(
                    'CMA_teststds option must have dimension = ' + str(N))
        if self.opts['CMA_diagonal']:  # is True or > 0
            # linear time and space complexity 
            self.B = array(
                1) # works fine with np.dot(self.B, anything) and self.B.T
            self.C = stds ** 2  # TODO: remove this!?
            self.dC = self.C
        else:
            self.B = np.eye(
                N) # identity(N), do not from matlib import *, as eye is a matrix there
        ...
        if not opts['seed']:
            ...
            opts['seed'] = 1e5 * np.random.rand() + six_decimals + 1e5 * (
            time.time() % 1)
        ...
        self.const.chiN = N ** 0.5 * (1 - 1. / (4. * N) + 1. / (
        21. * N ** 2)) # expectation of norm(randn(N,1))
        ...
        if opts['verb_disp'] > 0:
            ...
            print('(%d' % (self.sp.mu) + sweighted + ',%d' % (
            self.sp.popsize) + smirr + ')-CMA-ES' +
                  ' (mu_w=%2.1f,w_1=%d%%)' % (
                  self.sp.mueff, int(100 * self.sp.weights[0])) +
                  ' in dimension %d (seed=%d, %s)' % (
                  N, opts['seed'], time.asctime())) # + func.__name__
            if opts['CMA_diagonal'] and self.sp.CMA_on:
                ...
                if opts['CMA_diagonal'] is not True:
                    ...
                    s += ' iterations'
                    s += ' (1/ccov=' + str(
                        round(1. / (self.sp.c1 + self.sp.cmu))) + ')'
    ...
    def ask(self, number=None, xmean=None, sigma_fac=1):
        ...
        pop_pheno = [self.gp.pheno(x, copy=True, bounds=self.gp.bounds) for x in
                     pop_geno]
        ...
        if not self.gp.isidentity or use_sent_solutions:  # costs 25% in CPU performance with N,lambda=20,200
            ...
            if self.countiter % 30 / self.popsize ** 0.5 < 1:
                self.sent_solutions.truncate(0,
                                             self.countiter - 1 - 3 * self.N / self.popsize ** 0.5)
                # insert solutions
    ...
    def ask_geno(self, number=None, xmean=None, sigma_fac=1):
        ...
        if self.sp.CMA_on and (
                (self.opts['updatecovwait'] is None and
                         self.countiter >=
                             self.itereigenupdated + 1. / (
                             self.sp.c1 + self.sp.cmu) / self.N / 10
                ) or
                (self.opts['updatecovwait'] is not None and
                         self.countiter > self.itereigenupdated + self.opts[
                         'updatecovwait']
                )):
            self.updateBD()
        ...
        if 11 < 3:  # mutate along the principal axes only
            perm = np.random.permutation(
                self.N) # indices for mutated principal component
    ...
    def ask_and_eval(self, func, args=(), number=None, xmean=None, sigma_fac=1,
        ...
        for k in xrange(int(popsize)):
            ...
            while f in (np.NaN, None):  # rejection sampling
                ...
                if 11 < 3 and self.opts[
                    'vv'] and nreject < 2:  # trying out negative C-update as constraints handling
                    if not hasattr(self, 'constraints_paths'):
                        k = 1
                        self.constraints_paths = [np.zeros(self.N) for _i in
                                                  xrange(k)]
                ...
                if f not in (np.NaN, None) and evaluations > 1:
                    f = aggregation([f] + [func(x, *args) for _i in
                                           xrange(int(evaluations - 1))])
                if nreject + 1 % 1000 == 0:
                    print(
                        '  %d solutions rejected (f-value NaN or None) at iteration %d' %
    ...
    def tell(self, solutions, function_values, check_points=None, copy=False):
        ...
        if lam + self.sp.lam_mirr < 3:
            raise _Error('population size ' + str(
                lam) + ' is too small when option CMA_mirrors * popsize < 0.5')

        if not np.isscalar(function_values[0]):
            if np.isscalar(function_values[0][0]):
                if self.countiter <= 1:
                    print(
                        'WARNING: function values are not a list of scalars (further warnings are suppressed)')
                function_values = [val[0] for val in function_values]
            else:
                raise _Error(
                    'objective function values must be a list of scalars')
        ...
        self.out['recent_x'] = array(solutions[fit.idx[
            0]])  # TODO: change in a data structure(?) and use current as identify
        ...
        for s in solutions:  # use phenotype before Solution.repair() 
            if use_sent_solutions:
                x = self.sent_solutions.pop(s,
                                            None)  # 12.7s vs 11.3s with N,lambda=20,200
                if x is not None:
                    pop.append(x['geno'])
                ...
                # print 'WARNING: solution not found in ``self.sent_solutions`` (is expected for injected solutions)'
                    pop.append(self.gp.geno(s,
                                            copy=copy))  # cannot recover the original genotype with boundary handling
                    if check_points in (None, True, 1):
                        self.repair_genotype(pop[
                            -1])  # necessary if pop[-1] was changed or injected by the user.
        ...
        if 11 < 3 and self.opts[
            'vv'] and check_points:  # TODO: check_points might be an index-list
            cmean = self.sp.cmean / min(1, (sqrt(self.opts['vv'] * N) + 2) / (
            # abuse of cmean
        ...
        if 11 < 3:  # plot length of mean - mold
            self.more_to_write += [sqrt(sp.mueff) *
                                   sum(((1. / self.D) * dot(self.B.T,
                                                            self.mean - mold)) ** 2) ** 0.5 /
        ...
        c1a = c1 - (1 - hsig ** 2) * c1 * cc * (
        2 - cc)  # adjust for variance loss
            ...
            if not flgseparable:
                ...
                if 11 < 3: # ?3 to 5 times slower??
                    ...
                    for k in xrange(sp.mu):
                        z = (pop[k] - mold)
                        Z += np.outer((cmu * sp.weights[k] / (
                        self.sigma * self.sigma_vec) ** 2) * z, z)

                self.C *= 1 - c1a - cmu
                self.C += np.outer(c1 * self.pc, self.pc) + Z
                self.dC = np.diag(
                    self.C)  # for output and termination checking
                ...
                for k in xrange(sp.mu):
                    z = (pop[k] - mold) / (
                    self.sigma * self.sigma_vec) # TODO see above
                    Z += sp.weights[k] * z * z  # is 1-D
                self.C = (
                         1 - c1a - cmu) * self.C + c1 * self.pc * self.pc + cmu * Z
        ...
        if 1 < 3 and np.isfinite(sp.dampsvec):
            ...
            self.sigma_vec *= np.exp(
                (sp.cs / sp.dampsvec / 2) * (self.ps ** 2 - 1))
        ...
        # derandomized MSR = natural gradient descent using mean(z**2) instead of mu*mean(z)**2
            lengths = array(
                [sum(z ** 2) ** 0.5 for z in self.arz[fit.idx[:self.sp.mu]]])
            # print lengths[0::int(self.sp.mu/5)]
            self.sigma *= np.exp(self.sp.mueff ** 0.5 * dot(self.sp.weights,
                                                            lengths / self.const.chiN - 1)) ** (
                          2 / (N + 1))
        ...
        if 11 < 3 and self.opts['vv']:
            ...
            self.sigma = self.opts['vv'] * self.sp.mueff * sum(
                self.mean ** 2) ** 0.5 / N
        ...
        if self.sigma * min(self.dC) ** 0.5 < self.opts['minstd']:
            ...
            # g = self.countiter
        # N = self.N
        mindx = eval(self.opts['mindx']) if type(self.opts['mindx']) == type(
            '') else self.opts['mindx']
        ...
        if 11 < 3 and self.opts['verb_log'] > 0 and (self.countiter < 4 or
            ...
            CMADataLogger(self.opts['verb_filenameprefix']).register(self,
                                                                     append=True).add()
    ...
    def repair_genotype(self, x):
        ...
        if 1 < 3:  # hard clip at upper_length
            upper_length = self.N ** 0.5 + 2 * self.N / (
            self.N + 2)  # should become an Option, but how? e.g. [0, 2, 2]
            ...
            if fac > 1:
                ...
                return exp(
                    np.tanh(((upper_length * fac) ** 2 / self.N - 1) / 2) / 2)
        else:
    ...
    def updateBD(self):
        ...
        if self.sigma_vec is not 1 and not np.all(self.sigma_vec == 1):
            self.C = dot(dot(np.diag(self.sigma_vec), self.C),
                         np.diag(self.sigma_vec))
            self.sigma_vec[:] = 1
        ...
        if type(self.opts['CMA_eigenmethod']) == type(1):
            print(
                'WARNING: option CMA_eigenmethod should be a function, not an integer')
            ...
            # pygsl
                ...
                if 1 < 3:  # import pygsl on the fly
                    try:
                        ...
                        print(
                            'WARNING: could not find pygsl.eigen module, either install pygsl \n' +
                            '  or set option CMA_eigenmethod=1 (is much slower), option set to 1')
                        self.opts[
                            'CMA_eigenmethod'] = 0  # use 0 if 1 is too slow
            ...
            # TODO: thoroughly test np.linalg.eigh
                ...
                self.D, self.B = np.linalg.eigh(
                    self.C)  # self.B[i] is a row and not an eigenvector
        ...
        self.D = self.D[idx]
        self.B = self.B[:,
                 idx]  # self.B[i] is a row, columns self.B[:,i] are eigenvectors
        ...
        if 11 < 3 and any(abs(
                sum(self.B[:, 0:self.N - 1] * self.B[:, 1:], 0)) > 1e-6):
    ...
    def update_exponential(self, Z, eta, BDpair=None):
        ...
        Cs = dot(B, (B * D).T)
        self.C = dot(Cs, dot(
            Mh.expms(eta * dot(Csi, dot(Z, Csi)), self.opts['CMA_eigenmethod']),
            Cs))
    ...
    def feedForResume(self, X, function_values):
        ...
        if self.countiter > 0:
            print(
                'WARNING: feed should generally be used with a new object instance')
    ...
    def mahalanobisNorm(self, dx):
        ...
        """
        return sqrt(
            sum((self.D ** -1 * np.dot(self.B.T, dx)) ** 2)) / self.sigma
    ...
    #____________________________________________________________
    def disp(self,
             modulo=None):  # TODO: rather assign opt['verb_disp'] as default?
...
class Options(dict):
    ...
    def versatileOptions():
        ...
        """
        return tuple(sorted(i[0] for i in list(Options.defaults().items()) if
                            i[1].find(' #v ') > 0))
        ...
        if not unchecked:
            for key in list(self.keys()):
                if key not in Options.defaults():
                    print(
                        'Warning in cma.Options.__init__(): invalid key ``' + str(
                            key) + '`` popped')
                    self.pop(key)
    ...
    def set(self, dic, val=None, warn=True):
        ...
        for key, val in list(dic.items()):
            if key in Options.versatileOptions():
                ...
                print('Warning in cma.Options.set(): key ' + str(
                    key) + ' ignored')
    ...
    def __call__(self, key, default=None, loc=None):
        ...
        try:
            if type(val) is str:
                ...
                if type(val) == type('') and key.find(
                        'filename') < 0 and key.find('mindx') < 0:
    ...
    def set(self, opts, popsize=None, ccovfac=1, verbose=True):
        ...
        if popsize:
            ...
            popsize = opts.evalall({'N': N})[
                'popsize']  # the default popsize is computed in Options()
        sp.popsize = popsize
        if opts['CMA_mirrors'] < 0.5:
            ...
            sp.lam_mirr = int(0.5 + 0.16 * min((
            popsize, 2 * N + 2)) + 0.29)  # 0.158650... * popsize is optimal
            # lam = arange(2,22)
            # mirr = 0.16 + 0.29/lam
        ...
        if sp.mu > sp.popsize - 2 * sp.lam_mirr + 1:
            print("WARNING: pairwise selection is not implemented, therefore " +
                  " mu = %d > %d = %d - 2*%d + 1 = popsize - 2*mirr + 1 can produce a bias" % (
                      sp.mu, sp.popsize - 2 * sp.lam_mirr + 1, sp.popsize,
                      sp.lam_mirr))
        ...
        sp.weights = log(max([sp.mu, sp.popsize / 2.0]) + 0.5) - log(
            1 + np.arange(sp.mu))
        ...
        sp.dampsvec = opts['CMA_dampsvec_fac'] * (N + 2) if opts[
            'CMA_dampsvec_fac'] else np.Inf
        ...
        sp.cc = (4 + alpha_cc * sp.mueff / N) / (
        N + 4 + alpha_cc * 2 * sp.mueff / N)
        sp.cc_sep = (1 + 1 / N + alpha_cc * sp.mueff / N) / (
        N ** 0.5 + 1 / N + alpha_cc * 2 * sp.mueff / N) # \not\gg\cc
        sp.rankmualpha = opts['CMA_rankmualpha']
        # sp.rankmualpha = _evalOption(opts['CMA_rankmualpha'], 0.3)
        sp.c1 = ccovfac * min(1, sp.popsize / 6) * cone((N ** 2 + N) / 2,
                                                        sp.mueff,
                                                        N) # 2. / ((N+1.3)**2 + sp.mucov)
        sp.c1_sep = ccovfac * conedf(N, sp.mueff, N)
        if 11 < 3:
            sp.c1 = 0.
            print('c1 is zero')
        if opts['CMA_rankmu'] != 0:  # also empty
            sp.cmu = min(1 - sp.c1, ccovfac * cmu((N ** 2 + N) / 2, sp.mueff,
                                                  sp.rankmualpha))
            sp.cmu_sep = min(1 - sp.c1_sep,
                             ccovfac * cmudf(N, sp.mueff, sp.rankmualpha))
        else:
            sp.cmu = sp.cmu_sep = 0
        ...
        if opts['CMA_active']:
            ...
            # which means in particular weights should only be negative for q > 0.5+mirr_frac/2
            sp.neg.mu_f = popsize - (
            popsize + sp.lam_mirr) / 2 if popsize > 2 else 1
            sp.neg.weights = log(sp.mu_f + 0.5) - log(
                1 + np.arange(sp.popsize - int(sp.neg.mu_f), sp.popsize))
            sp.neg.mu = len(sp.neg.weights)  # maybe never useful?
            sp.neg.weights /= sum(sp.neg.weights)
            sp.neg.mueff = 1 / sum(sp.neg.weights ** 2)
            sp.neg.cmuexp = opts['CMA_activefac'] * 0.25 * sp.neg.mueff / (
            (N + 2) ** 1.5 + 2 * sp.neg.mueff)
        ...
        sp.damps = opts['CMA_dampfac'] * (0.5 +
                                          0.5 * min([1, (sp.lam_mirr / (
                                          0.159 * sp.popsize) - 1) ** 2]) ** 1 +
                                          2 * max([0, ((sp.mueff - 1) / (
                                          N + 1)) ** 0.5 - 1]) + sp.cs
        )
        if 11 < 3:
            # this is worse than damps = 1 + sp.cs for the (1,10000)-ES on 40D parabolic ridge
            sp.damps = 0.3 + 2 * max([sp.mueff / sp.popsize, ((sp.mueff - 1) / (
            N + 1)) ** 0.5 - 1]) + sp.cs
        if 11 < 3:
            # this does not work for lambda = 4*N^2 on the parabolic ridge
            sp.damps = opts['CMA_dampfac'] * (
            2 - 0 * sp.lam_mirr / sp.popsize) * sp.mueff / sp.popsize + 0.3 + sp.cs  # nicer future setting
...
class CMAStopDict(dict):
    ...
    def _update(self, es):
        ...
        if 11 < 3:  # 
            print(es.countiter, (
            opts['tolstagnation'], es.countiter > N * (5 + 100 / es.popsize),
            len(es.fit.histbest) > 100,
            np.median(es.fit.histmedian[:l]) >= np.median(
                es.fit.histmedian[l:2 * l]),
            np.median(es.fit.histbest[:l]) >= np.median(
                es.fit.histbest[l:2 * l])))
            # equality should handle flat fitness
        self._addstop('tolstagnation',
                      # leads sometimes early stop on ftablet, fcigtab, N>=50?
                      1 < 3 and opts['tolstagnation'] and es.countiter > N * (
                      5 + 100 / es.popsize) and
                      len(es.fit.histbest) > 100 and 2 * l < len(
                          es.fit.histbest) and
                      np.median(es.fit.histmedian[:l]) >= np.median(
                          es.fit.histmedian[l:2 * l]) and
                      np.median(es.fit.histbest[:l]) >= np.median(
                          es.fit.histbest[l:2 * l]))
        # iiinteger: stagnation termination can prevent to find the optimum

        self._addstop('tolupsigma', opts['tolupsigma'] and
                                    es.sigma / es.sigma0 / np.max(es.D) > opts[
                                        'tolupsigma'])

        if 11 < 3 and 2 * l < len(
                es.fit.histbest):  # TODO: this might go wrong, because the nb of written columns changes
            tmp = np.array((-np.median(es.fit.histmedian[:l]) + np.median(
                es.fit.histmedian[l:2 * l]),
                            -np.median(es.fit.histbest[:l]) + np.median(
                                es.fit.histbest[l:2 * l])))
            es.more_to_write += [(10 ** t if t < 0 else t + 1) for t in
                                 tmp] # the latter to get monotonicy
        ...
        if 1 < 3:
            ...
            self._addstop('noeffectcoord',
                          any([es.mean[i] == es.mean[i] + 0.2 * es.sigma * sqrt(
                              es.dC[i])
                               for i in xrange(N)]))
            if opts['CMA_diagonal'] is not True and es.countiter > opts[
                'CMA_diagonal']:
                i = es.countiter % N
                self._addstop('noeffectaxis',
                              sum(es.mean == es.mean + 0.1 * es.sigma * es.D[
                                  i] * es.B[:, i]) == N)
        ...
        if len(self):
            self._addstop(
                'flat fitness: please (re)consider how to compute the fitness more elaborate',
                len(es.fit.hist) > 9 and \
                max(es.fit.hist) == min(es.fit.hist))
        if 11 < 3 and opts['vv'] == 321:
            self._addstop('||xmean||^2<ftarget',
                          sum(es.mean ** 2) <= opts['ftarget'])
...
class BaseDataLogger2(DerivedDictBase):
    ...
    def disp(self):
        ...
        print(
            'method BaseDataLogger.disp() not implemented, to be done in subclass ' + str(
                type(self)))

    def plot(self):
        """plot data (not implemented)"""
        print(
            'method BaseDataLogger.plot() is not implemented, to be done in subclass ' + str(
                type(self)))

    def data(self):
        """return logged data in a dictionary (not implemented)"""
        print(
            'method BaseDataLogger.data() is not implemented, to be done in subclass ' + str(
                type(self)))
...
class CMADataLogger(BaseDataLogger):  # might become a dict at some point
    ...
    def __init__(self, name_prefix=default_prefix, modulo=1, append=False):
        ...
        # class properties:
        self.file_names = ('axlen', 'fit', 'stddev', 'xmean',
                           'xrecentbest') # used in load, however hard-coded in add
        self.key_names = ('D', 'f', 'std', 'xmean',
                          'xrecent') # used in load, however hard-coded in plot
        self.key_names_with_annotation = (
        'std', 'xmean', 'xrecent') # used in load
    ...
    def register(self, es, append=None, modulo=None):
        ...
        if type(es) != CMAEvolutionStrategy:
            raise TypeError(
                "only class CMAEvolutionStrategy can be registered for logging")
        self.es = es
    ...
    def initialize(self, modulo=None):
        ...
        try:
            with open(fn, 'w') as f:
                f.write(
                    '% # columns="iteration, evaluation, sigma, axis ratio, ' +
        ...
        try:
            f = open(fn, 'w')
            f.write(
                '%  columns="iteration, evaluation, sigma, max axis length, ' +
        ...
        try:
            with open(fn, 'w') as f:
                f.write(
                    '% # columns="iteration, evaluation, void, void, void, xmean", ' +
                    strseedtime)
            ...
            if self.key_names[i] in self.key_names_with_annotation:
                self.__dict__[self.key_names[i]].append(
                    self.__dict__[self.key_names[i]][
                        -1])  # copy last row to later fill in annotation position for display
            self.__dict__[self.key_names[i]] = array(
                self.__dict__[self.key_names[i]], copy=False)
        return self

    def add(self, es=None, more_data=[],
            modulo=None): # TODO: find a different way to communicate current x and f
        ...
        if es is None:
            try:
                ...
                raise _Error(
                    'call `add` with argument `es` or ``register(es)`` before ``add()``')
    ...
    def save(self, nameprefix, switch=False):
        ...
        if nameprefix == self.default_prefix:
            _Error(
                'cannot save to default name "' + nameprefix + '...", chose another name')

        if nameprefix == self.name_prefix:
            return

        for name in CMADataLogger.names:
            open(nameprefix + name + '.dat', 'w').write(
                open(self.name_prefix + name + '.dat').read())

        if switch:
            self.name_prefix = nameprefix

    def plot(self, fig=None, iabscissa=1, iteridx=None, plot_mean=True,
             # TODO: plot_mean default should be False
        ...
        # pylab: prodedural interface for matplotlib
            ...
            ImportError(
                'could not find matplotlib.pylab module, function plot() is not available')
            return
        ...
        if fontsize and pylab.rcParams['font.size'] != fontsize:
            ...
            pylab.rcParams[
                'font.size'] = fontsize  # subtracted in the end, but return can happen inbetween
        ...
        if iteridx is not None:
            ...
            dat.std = dat.std[
                      np.where([x in iteridx for x in dat.std[:, 0]])[0], :]
        ...
        if dat.x.shape[1] < 100:
            ...
            if x_opt is None:
                ...
                dat.x[-1, 5 + idx] = np.logspace(
                    np.log10(np.min(abs(dat.x[:, 5:]))),
        ...
        if dat.x.shape[1] < 100:
            ...
            if x_opt is not None:
                semilogy([dat.x[-1, iabscissa], ax[1]],
                         [abs(dat.x[-1, 5:]), yy[idx2]],
        ...
        if 1 < 2 and dat.std.shape[1] < 100:
            ...
            dat.std[-1, 5 + idx] = np.logspace(np.log10(np.min(dat.std[:, 5:])),
                                               np.log10(np.max(dat.std[:, 5:])),
                                               dat.std.shape[1] - 5)

            dat.std[-1, iabscissa] = minxend  # TODO: should be ax[1]
            yy = np.logspace(np.log10(ax[2]), np.log10(ax[3]),
                             dat.std.shape[1] - 5)
            ...
            plot(np.dot(dat.std[-2, iabscissa], [1, 1]),
                 array([np.min(dat.std[-2, 5:]), np.max(dat.std[-2, 5:])]),
                 'k-')
            hold(True)
    ...
    def plotdivers(dat, iabscissa, foffset):
        ...
        if dat.f.shape[1] > 8:
            ...
            semilogy(dat.f[:, iabscissa], np.abs(dat.f[:, 8:]) + 10 * foffset,
                     'm')
            hold(True)
        ...
        # overall minimum
        i = np.argmin(dat.f[:, 5])
        semilogy(dat.f[i, iabscissa] * np.ones(2), dat.f[i, 5] * np.ones(2),
                 'rd')
        ...
        semilogy(dat.std[:-1, iabscissa], np.vstack(
    ...
    def downsampling(self, factor=10, first=3, switch=True):
        ...
        for name in CMADataLogger.names:
            ...
            f.close()
            print('%d' % (
            cwritten) + ' lines written in ' + newprefix + name + '.dat')
        if switch:
    ...
    def disp(self, idx=100):  # r_[0:5,1e2:1e9:1e2,-10:0]):
        ...
        def printdatarow(dat, iteration):
            ...
            print('%5d' % (int(dat.f[i, 0])) + ' %6d' % (
            int(dat.f[i, 1])) + ' %.14e' % (dat.f[i, 5]) +
                  ' %5.1e' % (dat.f[i, 3]) +
                  ' %6.2e' % (max(dat.std[j, 5:])) + ' %6.2e' % min(
                dat.std[j, 5:]))
...
class DEAPCMADataLogger(BaseDataLogger):  # might become a dict at some point
    ...
    def load(self, filenameprefix=None):
        ...
        for key in ['xmean', 'std']:  # 'xrecent', 
            dat.__dict__[key].append(dat.__dict__[key][
                -1])  # copy last row to later fill in annotation position for display
    ...
    def add(self, fitness_values, es=None, more_data=[],
            modulo=None): # TODO: find a different way to communicate current x and f
        """append some logging data from `CMAEvolutionStrategy` class instance `es`, 
        if ``number_of_times_called % modulo`` equals to zero, never if ``modulo==0``. 
        ...
        if 11 < 3:
            try: # TODO: find a more decent interface to store and pass recent_x
                ...
                if self.counter == 2:  # by now a recent_x should be available
                    print(
                        'WARNING: es.out[\'recent_x\'] not found in CMADataLogger.add, count='
                        + str(self.counter))
        try:
        # fit
            ...
            with open(fn, 'a') as f:
                f.write(str(es.update_count) + ' '
                        ...
                        + str(es.sigma) + ' '
                        + '0 0 '
                        + ' '.join(map(str, es.sigma * np.sqrt(
                    [es.C[i][i] for i in xrange(es.dim)])))
                        + '\n')
                # xmean
            fn = self.name_prefix + 'xmean.dat'
            with open(fn, 'a') as f:
...
class AII(object):
    ...
    def tell(self, X, f):
        ...
        self.ps *= 1 - self.cs
        self.ps += (self.cs * (2 - self.cs)) ** 0.5 * mu ** 0.5 * np.mean(Z,
                                                                          axis=0)
        self.sigma *= np.exp(
            (self.cs / self.damps) * (sum(self.ps ** 2) ** 0.5 / self.chiN - 1))
        if self.use_abs_sigma:
            self.sigmai *= np.exp(
                (1 / self.dampi) * (np.abs(self.ps) / self.chi1 - 1))
        else:
        ...
        if self.use_scalar_product:
            if np.sign(
                    sum(self.r * (xmean - self.mean))) < 0: # and self.pr > 1:
...
def fmin(func, x0, sigma0=None, args=()
         ...
         # TODO: rename: iterwaitupdatedistribution?
         ,
    ...
    try: # pass on KeyboardInterrupt
        ...
        del opts[
            'sigma0']  # is not optional for the constructor CMAEvolutionStrategy
        ...
        del opts['restarts']
        noise_handling = Options({'noise_handling': noise_handling}).eval(
            'noise_handling')
        ...
        while 1:
            ...
            logger = CMADataLogger(opts['verb_filenameprefix'],
                                   opts['verb_log'])
            logger.register(es,
                            append).add()  # initial values, not fitness values
            ...
            noisehandler = NoiseHandler(es.N, noise_handling, np.median,
                                        opts['noise_reevals'],
                                        opts['noise_eps'],
                                        opts['eval_parallel'])
            while not es.stop():
                X, fit = es.ask_and_eval(func, args,
                ...
                if 11 < 3 and opts['vv']:  # inject a solution
                    ...
                    if 0 * np.random.randn() >= 0:
                        X[0] = 0 + opts['vv'] * es.sigma ** 0 * np.random.randn(
                            es.N)
                        fit[0] = func(X[0], *args)
                        # print fit[0]
                ...
                es.disp()
                logger.add(more_data=[noisehandler.evaluations,
                                      10 ** noisehandler.noiseS] if noise_handling else [],
                           modulo=1 if es.stop() and logger.modulo else None)
                if opts['verb_log'] and opts['verb_plot'] and \
                        (es.countiter % max(opts['verb_plot'], opts[
                            'verb_log']) == 0 or es.stop()):
            ...
            # final message
            if opts['verb_disp']:
                srestarts = (' after %i restart' + (
                's' if irun > 1 else '')) % irun if irun else ''
                for k, v in list(es.stop().items()):
                    print('termination on %s=%s%s (%s)' % (
                    k, str(v), srestarts, time.asctime()))

                print(
                    'final/bestever f-value = %e %e' % (es.best.last.f, best.f))
                if es.N < 9:
                    print('mean solution: ' + str(es.gp.pheno(es.mean)))
                    print('std deviation: ' + str(
                        es.sigma * sqrt(es.dC) * es.gp.scales))
                else:
                    print('mean solution: %s ...]' % (
                    str(es.gp.pheno(es.mean)[:8])[:-1]))
                    print('std deviations: %s ...]' % (
                    str((es.sigma * sqrt(es.dC) * es.gp.scales)[:8])[:-1]))
            ...
            opts[
                'popsize'] = incpopsize * es.sp.popsize # TODO: use rather options?
            opts['seed'] += 1

        # while irun

        es.out[
            'best'] = best  # TODO: this is a rather suboptimal type for inspection in the shell
        if 1 < 3:
        ...
        if opts['verb_disp'] > 0:
            print(
                ' in/outcomment ``raise`` in last line of cma.fmin to prevent/restore KeyboardInterrupt exception')
        raise # cave: swallowing this exception can silently mess up experiments, if ctrl-C is hit


def plot(name=None, fig=None, abscissa=1, iteridx=None, plot_mean=True,
         # TODO: plot_mean default should be False
         foffset=1e-19, x_opt=None, fontsize=10):
    """
    ...
    """
    CMADataLogger(name).plot(fig, abscissa, iteridx, plot_mean, foffset, x_opt,
                             fontsize)
...
class NoiseHandler(object):
    ...
    def __init__(self, N, maxevals=10, aggregate=np.median, reevals=None,
                 epsilon=1e-7, parallel=False):
        """parameters are
            `N`
                dimension
            `maxevals`
    ...
    def reeval(self, X, fit, func, ask, args=()):
        ...
        for i in self.idx:
            if self.epsilon:
                if self.parallel:
                    self.fitre[i] = fagg(
                        func(ask(evals, X[i], self.epsilon), *args))
                else:
                    self.fitre[i] = fagg(
                ...
                self.fitre[i] = fagg(
                    [func(X[i], *args) for _k in xrange(evals)])
    ...
    def update_measure(self):
        ...
        limits = [0.5 * (
        Mh.prctile(np.abs(r - (ranks[0, i] + 1 - (ranks[0, i] > ranks[1, i]))),
                   self.theta * 50) +
        ...
        s = np.abs(rankDelta[self.idx]) - Mh.amax(limits,
                                                  1)  # lives roughly in 0..2*lambda
...
class Sections(object):
    ...
    def __init__(self, func, x, args=(), basis=None, name=None,
        ...
        self.args = args
        self.x = x
        self.name = name if name else str(func).replace(' ', '_').replace('>',
                                                                          '').replace(
            '<', '')
    ...
    def plot(self, plot_cmd=None, tf=lambda y: y):
        ...
        for i in sorted(res.keys()):  # we plot not all values here
            if type(i) is int:
                ...
                plot_cmd(arx, [tf(np.median(res[i][x]) + addf) for x in arx],
                         color + '-')
    ...
    def __call__(self):
        ...
        if toc - self.tic >= self.lasttoc - self.tic:
            ...
            if self.messages < 3:
                self.messages += 1
                print(
                    '  in cma.ElapsedTime: time measure overflow, last difference estimated from',
                    self.tic0, self.tic, self.lasttoc, toc, toc - self.lasttoc,
                    self.lastdiff)
...
class Misc(object):
    ...
    class MathHelperFunctions(object):
        ...
        def amin(vec_or_scalar, vec_or_scalar2):
            return array(
                Misc.MathHelperFunctions.min(vec_or_scalar, vec_or_scalar2))
        ...
        def standard_finite_cauchy(size=1):
            ...
            if l == 0:
                return array(
                    [Mh.cauchy_with_variance_one() for _i in xrange(size)])
            elif l == 1:
                return array(
                    [Mh.cauchy_with_variance_one() for _i in xrange(size[0])])
            elif l == 2:
                return array(
    ...
    def loglikelihood(self, x, previous=False):
        ...
        if previous and hasattr(self, 'lastiter'):
            ...
            raise _Error(
                'no previous distribution parameters stored, check options importance_mixing')
        else:
            sigma = self.sigma
        ...
        res = -sum(dx ** 2) / sigma ** 2 / 2 - logs2pi - logdetC / 2 - n * log(
            sigma)
        if 1 < 3: # testing 
            s2pi = (2 * np.pi) ** (n / 2.)
            detC = np.prod(D) ** 2
            res2 = -sum(dx ** 2) / sigma ** 2 / 2 - log(
                s2pi * abs(detC) ** 0.5 * sigma ** n)
...
class Rotation(object):
    ...
    def __call__(self, x, inverse=False): # function when calling an object
        ...
        """
        N = x.shape[
            0]  # can be an array or matrix, TODO: accept also a list of arrays?
...
class FitnessFunctions(object):
    ...
    def noisysphere(self, x, noise=4.0, cond=1.0):
        ...
        return self.elli(x, cond=cond) * (
        1 + noise * np.random.randn() / len(x))
    ...
    def elli(self, x, rot=0, xoffset=0, cond=1e6, actuator_noise=0.0,
             both=False):
    ...
    def rosen(self, x, alpha=1e2):
        ...
        x = [x] if np.isscalar(x[0]) else x  # scalar into list
        f = [sum(alpha * (x[:-1] ** 2 - x[1:]) ** 2 + (1. - x[:-1]) ** 2) for x
             in x]
        return f if len(f) > 1 else f[0]  # 1-element-list into scalar
    ...
    def branin(self, x):
        ...
        x = x[0] + 5
        return (
               y - 5.1 * x ** 2 / 4 / np.pi ** 2 + 5 * x / np.pi - 6) ** 2 + 10 * (
        1 - 1 / 8 / np.pi) * np.cos(x) + 10 - 0.397887357729738160000
    ...
    def goldsteinprice(self, x):
        ...
        19 - 14 * x1 + 3 * x1 ** 2 - 14 * x2 + 6 * x1 * x2 + 3 * x2 ** 2)) * (
                   30 + (2 * x1 - 3 * x2) ** 2 * (
                   18 - 32 * x1 + 12 * x1 ** 2 + 48 * x2 - 36 * x1 * x2 + 27 * x2 ** 2)) - 3

    def griewank(self, x):
        # was in [-600 600]
        x = (600. / 5) * x
        return 1 - np.prod(np.cos(x / sqrt(1. + np.arange(len(x))))) + sum(
            x ** 2) / 4e3
    ...
    def rastrigin(self, x):
        ...
        if not np.isscalar(x[0]):
            N = len(x[0])
            return [10 * N + sum(xi ** 2 - 10 * np.cos(2 * np.pi * xi)) for xi
                    in x]
    ...
    def schwefelmult(self, x, pen_fac=1e4):
        ...
        N = len(y[0])
        f = array([
            418.9829 * N - 1.27275661e-5 * N - sum(x * np.sin(np.abs(x) ** 0.5))
...
def main(argv=None):
    ...
    if len(argv) >= 1:  # function and help
        if len(argv) == 1 or argv[1].startswith('-h') or argv[1].startswith(
                '--help'):
            print(main.__doc__)
            fun = None
            ...
            if len(argv) > 2 and (
                argv[2].startswith('--v') or argv[2].startswith(
                    '-v')):  # verbose
                print(
                    'doctest for cma.py: due to different platforms and python versions')
                print('and in some cases due to a missing unique random seed')
                print(
                    'many examples will "fail". This is OK, if they give a similar')
                ...
                print(
                    'doctest for cma.py: launching (it might be necessary to close a few pop up windows to finish)')
                ...
                process_test(fn)
                print(
                    'doctest for cma.py: finished (no other output should be seen after launching)')
            return
        elif argv[1] == '--doc':
            print(__doc__)
    ...
    if fun is not None:
        ...
        if fun is not None:
            tic = time.time()
            fmin(fun, x0, sig0,
                 **opts)  # ftarget=1e-9, tolfacupx=1e9, verb_log=10)
            # plot()
