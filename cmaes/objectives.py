...
class TwoBandObjective(Objective):
    ...
    def initialize_experiment_parameters(self):
        self.desired_lattice = DiffusionSquareLattice.load_configuration(
            './data/two_band_desired.ltc')
        self.dimensions = (
        self.desired_lattice.width, self.desired_lattice.height)
        self.resolution = self.desired_lattice.resolution
        self.stop_criterion = EnergyStopCriterion()
        self.max_difference = self.get_max_difference(
            self.desired_lattice) # max difference between lattices
    ...
    def error_function(self, pattern, lattice):
        ...
        for key in pattern.cells.keys():
            difference += (pattern.cells[key].state.grayscale - lattice.cells[
